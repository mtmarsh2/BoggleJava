...
/**
 * Easy gameoflife for Java students and programmers interested in creating Java
...
public class Zen extends JApplet {
	...
	class ZenInstance {
		...
		public Graphics2D getBufferGraphics() {
			...
			while (g == null) {
				System.err
						.println("Odd... gameoflife not yet ready! Sleeping...");
		...
		void createBuffers(int width, int height, String options) {
			...
			master.flipBuffer();// set up gameoflife, including font and color
			// state
	...
	private void paintWindowImmediately() {
		...
		if (VERBOSE)
			System.err.println("paintWindowImmediately gameoflife="
					+ windowGraphics);
		if (windowGraphics != null)
			paint(getGraphics());
		else
